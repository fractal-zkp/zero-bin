...
# 3 --> Rpc type (eg. jerigon / native)
# 4 --> Backoff time (in milliseconds) for the RPC requests
# 5 --> Number of retries for the RPC requests
...
cargo r --release --features test_only --bin leader -- -n 1 --runtime in-memory "$3" --rpc-url "$2" --block-number "$1" --checkpoint-block-number "$(($1-1))" --proof-output-path $OUT_DUMMY_PROOF_PATH --backoff "${4:-0}" --max-retries "${5:-0}" > $OUT_LOG_PATH 2>&1
retVal=$?
if [ $retVal -ne 0 ]; then
    # Some error occured.
